...
@section('title', 'Tambah WhatsApp Template')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="card p-6">
        <h2 class="text-base font-semibold leading-7 text-slate-900 dark:text-white">Tambah WhatsApp Template</h2>
        <p class="mt-1 text-sm leading-6 text-slate-600 dark:text-slate-400">Buat template pesan WhatsApp baru. Anda dapat menyisipkan variabel dinamis menggunakan kurung kurawal `{nama_variabel}`.</p>

        <form action="{{ route('communication.whatsapp-templates.store') }}" method="POST" class="mt-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-2">
                <div>
                    ...
                    <div class="mt-2">
                        <input type="text" name="name" id="name" required value="{{ old('name') }}" class="input-field" placeholder="contoh: Pengingat Kontrol H+1">
                    </div>
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <select name="type" id="type" required class="input-field select-field">
                            <option value="follow_up" {{ old('type') === 'follow_up' ? 'selected' : '' }}>Follow Up / Rencana Kontrol</option>
                            <option value="appointment" {{ old('type') === 'appointment' ? 'selected' : '' }}>Kunjungan Baru / Janji Temu</option>
                            <option value="custom" {{ old('type') === 'custom' ? 'selected' : '' }}>Kustom Umum</option>
                        </select>
                    </div>
            ...
            <div class="flex flex-col gap-y-3 sm:flex-row sm:items-center sm:gap-x-6">
                <div class="flex items-center gap-x-3">
                    <input type="checkbox" name="is_default" id="is_default" value="1" {{ old('is_default') ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-600 dark:border-slate-600 dark:bg-slate-700">
