...
@section('title', 'Edit Group Akses')

@section('content')
<div class="sm:flex sm:items-center">
    <div class="sm:flex-auto">
        <h1 class="text-base font-semibold leading-6 text-slate-900 dark:text-white">Edit Group Akses</h1>
        <p class="mt-2 text-sm text-slate-700 dark:text-slate-400">Ubah peran dan atur izin hak aksesnya.</p>
    </div>
</div>

<form action="{{ route('administration.roles.update', $role) }}" method="POST" class="mt-8 space-y-8 max-w-6xl">
    @csrf
    @method('PUT')

    @if($isBuiltin)
        <div class="rounded-md bg-blue-50 dark:bg-blue-900/30 p-4 ring-1 ring-blue-500/20 max-w-xl">
            <div class="flex">
                <div class="shrink-0">
                    <svg class="h-5 w-5 text-blue-500" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.25v2.75a.75.75 0 001.5 0V10A.75.75 0 0010 9H9z" clip-rule="evenodd" />
                    </svg>
                </div>
                <div class="ml-3">
                    <p class="text-xs text-blue-800 dark:text-blue-200">
                        <strong>Peran Bawaan Sistem</strong>: Anda tidak dapat mengubah nama group akses bawaan ini demi menjaga stabilitas sistem. Namun, Anda tetap dapat menyesuaikan hak akses (permission) di bawah ini.
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="card p-6 bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700/50">
        <div>
            ...
            <div class="mt-2">
                @if($isBuiltin)
                    <input type="text" value="{{ ucwords(str_replace('-', ' ', $role->name)) }}" class="block w-full max-w-md rounded-md border-0 py-1.5 text-slate-500 bg-slate-50 dark:bg-slate-900/50 dark:text-slate-400 shadow-sm ring-1 ring-inset ring-gray-200 sm:text-sm sm:leading-6 dark:ring-slate-700 cursor-not-allowed" readonly>
                    <input type="hidden" name="name" value="{{ $role->name }}">
                @else
                    <input type="text" name="name" id="name" value="{{ old('name', ucwords(str_replace('-', ' ', $role->name))) }}" placeholder="Contoh: Staff Keuangan, Admin Penjualan" class="block w-full max-w-md rounded-md border-0 py-1.5 text-slate-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-emerald-600 sm:text-sm sm:leading-6 dark:bg-slate-900 dark:text-white dark:ring-slate-700" required>
                @endif
            </div>
            @error('name')
    ...
    <div>
        <div class="flex items-center justify-between">
            ...
            <button type="button" id="check-all" class="text-xs font-semibold text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 font-medium">Pilih Semua</button>
        </div>
                            ...
                            @foreach($permissions as $permissionKey => $permissionLabel)
                                <label class="relative flex items-start cursor-pointer group">
                                    <div class="flex h-6 items-center">
                                        <input type="checkbox" name="permissions[]" value="{{ $permissionKey }}" @checked(in_array($permissionKey, $rolePermissions)) class="permission-checkbox h-4 w-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-600 dark:border-slate-700 dark:bg-slate-900" @change="updateCount()">
                                    </div>
