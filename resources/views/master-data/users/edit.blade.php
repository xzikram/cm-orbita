...
<div class="max-w-3xl mx-auto">
    <div class="sm:flex sm:items-center sm:justify-between mb-6">
        <div class="sm:flex-auto">
            <h1 class="text-base font-semibold leading-6 text-slate-900 dark:text-white">Edit User</h1>
            <p class="mt-2 text-sm text-slate-700 dark:text-slate-400">Perbarui data pengguna sistem.</p>
        </div>
    ...
    <div class="card p-6 bg-white dark:bg-slate-800 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700/50">
        <form action="{{ route('administration.users.update', $user) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <input type="text" name="name" id="name" required value="{{ old('name', $user->name) }}" class="input-field" placeholder="Nama lengkap user">
                    </div>
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <input type="text" name="nik" id="nik" value="{{ old('nik', $user->nik) }}" class="input-field" placeholder="Contoh: 3173123456780001">
                    </div>
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <input type="email" name="email" id="email" required value="{{ old('email', $user->email) }}" class="input-field" placeholder="email@domain.com">
                    </div>
                ...
                <div>
                    ...
                    <div class="mt-2">
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $user->phone) }}" class="input-field" placeholder="Misal: 08123456789">
                    </div>
                ...
                <!-- Password -->
                <div>
                    <label for="password" class="block text-sm font-medium leading-6 text-slate-900 dark:text-slate-200">Password Baru</label>
                    <div class="mt-2">
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium leading-6 text-slate-900 dark:text-slate-200">Konfirmasi Password Baru</label>
                    <div class="mt-2">
                        <input type="password" name="password_confirmation" id="password_confirmation" class="input-field" placeholder="Kosongkan jika tidak diubah">
                    </div>
                </div>
                        ...
                        @if($isSuperAdmin)
                            <select name="clinic_id" id="clinic_id" class="input-field" required>
                                ...
                                @foreach($clinics as $clinic)
                                    <option value="{{ $clinic->id }}" @selected(old('clinic_id', $user->clinic_id) == $clinic->id)>
                                        {{ $clinic->name }}
                                    </option>
                                @endforeach
                            </select>
                        @else
                            <input type="text" value="{{ $user->clinic?->name }}" class="input-field bg-slate-50 dark:bg-slate-900/50 text-slate-500 dark:text-slate-400 cursor-not-allowed" disabled>
                        @endif
                    </div>
                ...
                <div class="sm:col-span-2">
                    <div class="relative flex items-start">
                        <div class="flex h-6 items-center">
                            <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $user->is_active) ? 'checked' : '' }} class="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-600 dark:border-slate-700 dark:bg-slate-900">
                        </div>
